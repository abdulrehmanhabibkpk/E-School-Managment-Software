  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      ...
      printWindow.document.write(`
        <html dir="rtl">
          <head>
            <title>امتحانی حاضری شیٹ - ${examClass}</title>
            ...
            <style>
              ...
              th, td { border: 1.5px solid black !important; }
              @media print {
              ...
              return (
                <tr key={idx} className="h-16">
                  ...
                  <td className="border-[1.5px] border-black flex justify-center items-center p-1">
                    {student ? <QRCodeSVG value={profileUrl} size={40} /> : null}
                  </td>
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-white rounded-[2rem] w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl overflow-hidden font-urdu">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          ...
          <div className="flex gap-3">
             <button onClick={handlePrint} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2">
