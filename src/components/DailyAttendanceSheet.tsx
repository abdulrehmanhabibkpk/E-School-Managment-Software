    ...
    <div className="min-h-screen bg-slate-50 font-urdu">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm no-print">
        <div className="flex items-center gap-4">
          <button 
            onClick={onBack}
            className="bg-slate-800 text-white px-6 py-2 rounded-xl text-sm font-bold flex flex-col items-center justify-center gap-0 hover:bg-slate-900 transition-all shadow-lg shadow-slate-800/20"
          >
            <div className="flex items-center gap-2">
              <ArrowRight className="w-4 h-4" />
              <span>واپس جائیں</span>
            </div>
            <span className="text-[10px] font-normal opacity-70">Back</span>
          </button>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex flex-col gap-1">
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest px-1">Class (درجہ)</span>
              <select 
                value={selectedClass} 
                onChange={e => setSelectedClass(e.target.value)}
                className="bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500 min-w-[120px]"
              >
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest px-1">Teacher (استاد)</span>
              <select 
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest px-1">Book (کتاب)</span>
              <select 
                value={selectedBook} 
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest px-1">Time (وقت)</span>
              <select 
                value={selectedTime} 
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest px-1">Month (مہینہ)</span>
              <select 
                value={selectedMonth} 
            ...
            <div className="flex flex-col gap-1">
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest px-1">Year (سال)</span>
              <select 
                value={selectedYear} 
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className={`px-8 py-2 rounded-xl text-sm font-bold flex flex-col items-center justify-center gap-0 transition-all ${selectedClass ? 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 active:scale-95' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
          >
            <div className="flex items-center gap-2">
      ...
      <div className="max-w-[1200px] mx-auto p-8 no-print">
        {!selectedClass ? (
          <div className="bg-white rounded-[40px] p-20 text-center border-2 border-dashed border-slate-200">
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-4">Daily Attendance Sheet Generator</p>
            <p className="text-slate-500 font-urdu">براہ کرم حاضری شیٹ دیکھنے کے لیے درجہ منتخب کریں</p>
            <span className="text-[10px] text-slate-300">Please select a class to view the attendance sheet preview.</span>
          </div>
        ) : (
