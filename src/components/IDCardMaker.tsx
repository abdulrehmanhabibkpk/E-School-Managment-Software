...
export default function IDCardMaker({ onBack }: IDCardMakerProps) {
  ...
  const [cardData, setCardData] = useState({
    ...
    signature: '',
    phone: '0300-1234567',
    address: 'Main City Academic Zone'
  });
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] p-4 md:p-8 flex flex-col items-center gap-8 font-sans" dir="ltr">
      {/* Header Panel */}
      <div className="w-full max-w-[1200px] no-print flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-[32px] shadow-xl shadow-blue-900/5 border border-white relative overflow-hidden">
        <div className="absolute top-0 left-0 w-32 h-32 bg-blue-50 rounded-full -translate-y-16 -translate-x-16 opacity-50" />
        ...
        <div className="flex items-center gap-6 relative z-10 w-full md:w-auto">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Student ID Card Center</h2>
            <p className="text-xs text-slate-400 font-bold">Professional Student ID Card Generator</p>
          </div>
        </div>
      ...
      <div className="w-full max-w-[1200px] grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
        ...
        <div className="lg:col-span-4 space-y-8 no-print">
          ...
          <div className="bg-white p-6 rounded-[32px] shadow-xl shadow-blue-900/5 border border-white">
            ...
            <div className="relative mb-6">
              <input 
                ...
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl pl-12 pr-4 py-4 text-sm focus:border-blue-500 outline-none transition-all font-bold"
              />
              ...
              {filteredStudents.map(student => (
                ...
                >
                  ...
                  <div className="text-left flex-1">
                    ...
                    <div className="text-[10px] text-slate-500 font-bold">Roll No: {student.rollNo} | {student.grade}</div>
                  </div>
                </button>
              ))}
          ...
          <div className="bg-white p-6 rounded-[32px] shadow-xl shadow-blue-900/5 border border-white">
            ...
            >
              ...
              <span className="text-[10px] text-slate-400 font-bold">Upload new student photo</span>
              <input 
                ref={fileInputRef}
            ...
            >
              {cardData.signature ? (
                ...
                <>
                  ...
                  <span className="text-[10px] text-slate-400 font-bold">Upload signature image</span>
                </>
              )}
              <input 
        ...
        <div className="lg:col-span-8 flex flex-col items-center gap-12">
          ...
          <div className="bg-orange-50 text-orange-700 px-6 py-3 rounded-2xl text-[10px] font-bold border border-orange-100 no-print">
            Note: On printing, only the ID card will be printed.
          </div>

          <div className="id-card-render-container flex flex-col md:flex-row gap-12 print:gap-4 print:p-0">
            
            {/* FRONT SIDE */}
            <div className="w-[85.6mm] h-[54mm] bg-[#FDFDEE] rounded-[2mm] shadow-[0_15px_35px_rgba(0,0,0,0.2)] relative overflow-hidden flex flex-col border border-slate-300 print:shadow-none print:border-slate-400 font-sans">
               ...
               <div className="absolute top-2 left-4 z-20">
                  <div className="relative">
                    <div className="bg-[#FFF9C4] border border-orange-200 px-6 py-1 rounded-sm shadow-sm transform skew-x-12 flex items-center justify-center">
                      <span className="text-[10px] font-black text-blue-800 transform -skew-x-12">STUDENT CARD</span>
                    </div>
                  </div>
               </div>
               ...
               <div className="flex-1 flex p-4 gap-4 relative z-10 mt-4">
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center pt-4">
                     <div className="bg-gradient-to-b from-blue-700 to-blue-900 border-2 border-blue-400 rounded-lg px-2 py-1.5 shadow-lg w-full text-center relative">
                        <h2 className="text-[12px] font-black text-white leading-tight drop-shadow-md">{getMadrassaName()}</h2>
                        <span className="absolute -top-1 -right-1 bg-yellow-400 text-[6px] text-blue-900 px-1 rounded-full border border-blue-900 font-black">Official</span>
                     </div>
                     
                     <div className="w-full bg-[#008037] text-white text-[7px] font-bold py-0.5 mt-2 rounded-sm shadow-sm border border-green-800 text-center">
                        Academic Institute & Educational System
                     </div>
                     ...
                     <div className="mt-2 w-32 h-10 border border-blue-400 bg-white/80 rounded-sm relative overflow-hidden flex items-center justify-center p-1">
                        {cardData.signature ? (
                           ...
                           <div className="absolute inset-0 flex items-center justify-center opacity-70">
                              <span className="font-serif text-blue-600 text-[10px]">{cardData.name}</span>
                           </div>
                        )}
                        <div className="absolute bottom-0 right-0 px-1 bg-white text-[5px] text-blue-800 border-t border-l border-blue-400">Signature</div>
                     </div>
                  </div>
               </div>
               ...
               <div className="h-6 bg-[#A5D6A7] flex items-center justify-center px-4 border-t border-green-600">
                  <span className="text-[7px] font-black text-slate-800">If found, please return to the institute office address.</span>
               </div>
            </div>

            {/* BACK SIDE */}
            <div className="w-[85.6mm] h-[54mm] bg-[#FDFDEE] rounded-[2mm] shadow-[0_15px_35px_rgba(0,0,0,0.2)] relative overflow-hidden flex flex-col border border-slate-300 print:shadow-none print:border-slate-400 p-4 font-sans text-left">
               
               {/* Background Watermark */}
               <div className="absolute inset-0 opacity-[0.05] pointer-events-none flex items-center justify-center">
                  <div className="text-[30px] font-black text-slate-400 rotate-[-45deg] uppercase">Academic Institute</div>
               </div>

               {/* ID Box Row */}
               <div className="flex items-center gap-2 mb-2 justify-start">
                  <span className="text-[8px] font-black text-green-700 leading-tight">CNIC / ID No:</span>
               ...
               <div className="space-y-1.5 relative z-10">
                  <div className="flex border-b border-dotted border-slate-400 pb-0.5">
                     <span className="text-[8px] font-black text-red-700 w-24">Full Name:</span>
                     <span className="text-[9px] font-black text-slate-800 flex-1 text-left text-blue-800">{cardData.name} S/D/O {cardData.fatherName}</span>
                  </div>
                  <div className="flex border-b border-dotted border-slate-400 pb-0.5">
                     <span className="text-[8px] font-black text-red-700 w-24">Address:</span>
                     <span className="text-[9px] font-black text-slate-800 flex-1 text-left text-blue-800">{cardData.address}</span>
                  </div>
                  <div className="flex items-center gap-4 border-b border-dotted border-slate-400 pb-0.5">
                     <div className="flex flex-1">
                        <span className="text-[8px] font-black text-red-700 w-20">DOB:</span>
                        <span className="text-[9px] font-black text-slate-800 flex-1 text-left text-blue-800">{cardData.dob || '---'}</span>
                     </div>
                     <div className="flex flex-1">
                        <span className="text-[8px] font-black text-green-700 w-20">Roll No:</span>
                        <span className="text-[9px] font-black text-slate-800 flex-1 text-left text-blue-800">{cardData.studentId}</span>
                     </div>
                  </div>
                  <div className="flex items-center gap-4 border-b border-dotted border-slate-400 pb-0.5">
                     <div className="flex flex-1">
                        <span className="text-[8px] font-black text-red-700 w-20">Class/Grade:</span>
                        <span className="text-[9px] font-black text-slate-800 flex-1 text-left text-blue-800">{cardData.department}</span>
                     </div>
                     <div className="flex flex-1">
                        <span className="text-[8px] font-black text-green-700 w-20">Blood Group:</span>
                        <span className="text-[9px] font-black text-slate-800 flex-1 text-left text-blue-800">{cardData.bloodGroup}</span>
                     </div>
                  </div>
               </div>
               ...
               <div className="mt-2 text-[8px] font-black text-slate-800 text-center">
                  Card Valid From <span className="text-blue-800">01 Apr {new Date().getFullYear()}</span> To <span className="text-blue-800">31 Mar {new Date().getFullYear()+1}</span>
               </div>
               ...
               <div className="mt-auto -mx-4 -mb-4 bg-[#008037] text-white p-1.5 flex items-center justify-between px-4">
                  ...
                  <div className="flex flex-col items-center">
                     <span className="text-[9px] font-black font-sans">{getMadrassaName()}</span>
                  </div>
                  <div className="bg-white text-green-800 text-[6px] font-black px-2 py-0.5 rounded-sm">Official ID</div>
               </div>
            </div>
      ...
      <div className="w-full max-w-[1200px] no-print bg-white p-8 rounded-[32px] shadow-xl shadow-blue-900/5 border border-white mb-20">
         <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-black text-slate-800">Edit Card Details</h3>
            <div className="px-4 py-1.5 bg-blue-50 text-blue-700 rounded-full text-xs font-black">Manual Override</div>
         </div>
         <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase">Grade / Class</label>
