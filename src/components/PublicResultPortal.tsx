...
export default function PublicResultPortal({ onClose }: PublicResultPortalProps) {
  ...
  const [examTypesList, setExamTypesList] = useState<string[]>([
    'سالانہ',
    'جائزہ / ہفتہ وار',
    'ضمنی',
    'سہ ماہی',
  ...
  const [targetSettings, setTargetSettings] = useState<any>({
    jamiaName: 'الجامعہ عربیہ سراج العلوم جبوری مانسہرہ',
    monogram: ''
  });
  ...
  useEffect(() => {
    async function fetchMadrasas() {
      ...
      try {
        ...
        if (!list || list.length === 0) {
          ...
          const sysName = sysSaved ? JSON.parse(sysSaved).jamiaName : 'الجامعہ عربیہ سراج العلوم جبوری مانسہرہ';
    ...
    try {
      ...
      const combinedGrades = [...savedGradesList, ...savedGrades];
      const gradeNames = Array.from(new Set(combinedGrades.map((g: any) => typeof g === 'string' ? g : (g.name || '')).filter(Boolean)));
      if (gradeNames.length > 0) {
        setClassesList(gradeNames);
        setSelectedClass(gradeNames[0]);
      }
    } catch(e) {
      console.warn('Could not load dynamic grades in PublicResultPortal:', e);
    }

    // Load custom exam types
    try {
      const savedExams = JSON.parse(localStorage.getItem('exams') || '[]');
      const defaultExams = savedExams.length > 0 ? savedExams : ['سالانہ', 'جائزہ', 'ضمنی', 'سہ ماہی', 'ششماہی', 'ماہانہ'];
      ...
      if (combinedExams.length > 0) {
        setExamTypesList(combinedExams);
      }
    } catch(e) {
      console.warn('Could not load dynamic exam types in PublicResultPortal:', e);
    }
  }, []);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchedDocs.forEach((fDoc) => {
        ...
        matches.forEach((student: any) => {
          tempResults.push({
            ...
            madrassaData,
            madrassaId: fDoc.id,
            madrassaName: fDoc.data?.system_settings?.jamiaName || 'الجامعہ عربیہ سراج العلوم جبوری مانسہرہ'
          });
        });
      });
        ...
        localMatches.forEach((student: any) => {
          ...
          if (!alreadyAdded) {
            tempResults.push({
              ...
              madrassaName: localSystemSettings.jamiaName || 'الجامعہ عربیہ سراج العلوم جبوری مانسہرہ'
            });
          }
        });
      } catch (locErr) {
      ...
      setTargetSettings({
        jamiaName: systemSet.jamiaName || 'الجامعہ عربیہ سراج العلوم جبوری مانسہرہ',
        ...
        <div className="bg-[#800000] text-white px-6 py-5 flex items-center justify-between shadow-md">
          <div className="flex items-center gap-3">
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-white/80 font-medium">الجامعہ عربیہ سراج العلوم جبوری مانسہرہ — تعلیمی تعمیل پورٹل</p>
            </div>
          </div>
          <button 
            onClick={onClose}
