      ...
      try {
        ...
        recognitionRef.current.onerror = (event: any) => {
          ...
          if (event.error === 'not-allowed') {
            setError('مائیکروفون کی اجازت نہیں ملی۔');
          } else {
            setError('وائس ٹائپنگ میں مسئلہ آگیا۔');
          }
        };
  ...
  return (
    <div className="relative inline-flex items-center gap-1">
      <button
        ...
        title={error || (isListening ? 'بولنا بند کریں' : 'وائس ٹائپنگ')}
      >
        {isListening ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4" />}
        {error && !isListening && (
          <span className="absolute -top-1 -right-1 w-2 h-2 bg-amber-500 rounded-full animate-bounce"></span>
        )}
      </button>
